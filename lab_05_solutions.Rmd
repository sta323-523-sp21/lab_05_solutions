...
x <- c("1 name available?", "2 names available?", "no names found!",
       "22 names available?")
str_match(x, pattern = "\\d\\d? names? available\\?")
```
...
str_match(x, pattern = "^M.*successful$")
```
...
str_match(x, pattern = "1?\\s?\\(?\\d{3}\\)?[\\s-]?\\d{3}[\\s-]?\\d{4}")
```
